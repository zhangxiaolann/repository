...
$token=authcode("{$gl}\t{$session}", 'ENCODE');

if(isset($gl) && isset($pa)) {
$row = $DB->get_row("SELECT * FROM wjob_user WHERE user='$gl' limit 1");
if($row['user']=='') {
...
{
	$token=authcode(daddslashes($_COOKIE['token']), 'DECODE');
	list($gl, $sid) = explode("\t", $token);
	$DB->query("UPDATE wjob_user SET last='$date',dlip='$clientip' WHERE user = '$gl'");
	$row = $DB->get_row("SELECT * FROM wjob_user WHERE user='$gl' limit 1");
	$session=md5($row['user'].$row['pass'].$password_hash);
	if($session==$sid)$islogin=1;
	if($conf['adminid']==$row['userid'])$isadmin=1;
