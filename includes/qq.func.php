...
global $DB,$siteurl,$qqapi_server,$getss;
...
case 'zan':
	...
	$url=$qqapi_server.'qq/zan.php?qq='.$qq.'&sid='.$row['sid'].'&skey='.$row['skey'].'&pskey='.$row['pskey'].'&pskey2='.$row['pskey2'].'&method='.$qqrow['method'].'&forbid='.$qqrow['forbid'].$extend;
	$info='QQ：<u>'.$qq.'</u><br/>秒赞协议：<u>'.$methodarr[$method].'</u><br/>不秒赞以下QQ：<u>'.$qqrow['forbid'].'</u>';
break;
case 'pl':
	if($row['status2']!=1 && $qqrow['method']>=3)$qqrow['method']=2;
	$url=$qqapi_server.'qq/pl.php?qq='.$qq.'&sid='.$row['sid'].'&skey='.$row['skey'].'&pskey='.$row['pskey'].'&pskey2='.$row['pskey2'].'&method='.$qqrow['method'].'&forbid='.$qqrow['forbid'].'&content='.$qqrow['content'].$extend;
	$info='QQ：<u>'.$qq.'</u><br/>秒评协议：<u>'.$methodarr[$method].'</u><br/>评论内容：<u>'.$qqrow['content'].'</u><br/>不秒评以下QQ：<u>'.$qqrow['forbid'].'</u>';
break;
case 'qqsign':
	if($row['status2']!=1 && $qqrow['method']>=3)$qqrow['method']=2;
	$url=$qqapi_server.'qq/qqsign.php?qq='.$qq.'&sid='.$row['sid'].'&skey='.$row['skey'].'&pskey='.$row['pskey'].'&pskey2='.$row['pskey2'].'&method='.$qqrow['method'];
	$info='QQ：<u>'.$qq.'</u><br/>签到协议：<u>'.$methodarr[$method].'</u>';
break;
case 'qqss':
	if($row['status2']!=1 && $qqrow['method']>=3)$qqrow['method']=2;
	$url=$qqapi_server.'qq/ss.php?qq='.$qq.'&sid='.$row['sid'].'&skey='.$row['skey'].'&pskey='.$row['pskey'].'&pskey2='.$row['pskey2'].'&method='.$qqrow['method'].'&nr='.urlencode($qqrow['nr']).'&img='.urlencode($qqrow['img']).'&ua='.urlencode($qqrow['ua']);
	$info='QQ：<u>'.$qq.'</u><br/>运行协议：<u>'.$methodarr[$method].'</u><br/>内容：<u>'.$qqrow['nr'].'</u><br/>图片地址：<u>'.$qqrow['img'].'</u><br/>浏览器UA：<u>'.$qqrow['ua'].'</u>';
break;
case 'ht':
	$url=$qqapi_server.'qq/ht.php?qq='.$qq.'&sid='.$row['sid'].'&skey='.$row['skey'];
	$info='QQ：<u>'.$qq.'</u><br/>正在花藤挂机中。';
break;
case 'del':
	if($row['status2']!=1 && $qqrow['method']>=3)$qqrow['method']=2;
	$url=$qqapi_server.'qq/del.php?qq='.$qq.'&sid='.$row['sid'].'&skey='.$row['skey'].'&pskey='.$row['pskey'].'&pskey2='.$row['pskey2'].'&method='.$qqrow['method'];
	$info='QQ：<u>'.$qq.'</u><br/>运行协议：<u>'.$methodarr[$method].'</u>';
break;
case 'zfss':
	$url=$qqapi_server.'qq/zfss.php?qq='.$qq.'&sid='.$row['sid'].'&skey='.$row['skey'].'&pskey='.$row['pskey'].'&pskey2='.$row['pskey2'].'&method='.$qqrow['method'].'&uin='.$qqrow['uin'].'&nr='.$qqrow['reason'].$extend;
...
break;
case 'delll':
	$url=$qqapi_server.'qq/delll.php?qq='.$qq.'&sid='.$row['sid'].'&skey='.$row['skey'].'&pskey='.$row['pskey'].'&pskey2='.$row['pskey2'];
