...
$sql="TRUNCATE TABLE `wjob_user`;
TRUNCATE TABLE `wjob_job`;
TRUNCATE TABLE `wjob_qq`";
$sql=explode(';',$sql);
$t=0;$e=0;
...
$DB->query("insert into `wjob_user` (`pass`,`user`,`date`,`last`) values ('".$row['pass']."','".$row['user']."','".$date."','".$date."')");
...
$sql="DELETE FROM wjob_user WHERE num='0' and userid!='1'";
if($DB->query($sql))
...
elseif($my == 'qlrw_ok'){
$sql="delete from `wjob_job` where `url` LIKE '%{$_GET['kw']}%'";
if($DB->query($sql))
...
elseif($my == 'qlqq_ok'){
$sql="delete from `wjob_qq` where `status`='5' or `status`='4'";
if($DB->query($sql))
