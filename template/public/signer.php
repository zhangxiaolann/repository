...
for($i=0;$i<$conf['sysnum'];$i++){
	...
	$addstr='';
	$addstr2='';
	$all_sys=$DB->count("SELECT count(*) from wjob_job WHERE sysid='$sysid'");
...
echo '<a href="index.php?mod=signer&my=pwsign&sys='.$sysid.$link.'"><img src="images/icon/pw.ico" class="logo">添加phpwind自动签到任务</a><hr/>';
...
elseif($_GET['my']=='pwsign'){
$signurl = ($apiserverid!=0) ? $apiserver.'sign/pwsign/do.php?backurl='.$baseUrl : '../sign/pwsign.php';
echo <<<HTML
<div class="w h"><h3>添加PHPWind9.X自动签到任务</h3></div>
<div class="box">
<form action="{$signurl}" method="POST">
[密码模式][<a href="index.php?mod=signer&my=pwsign2&sys={$sysid}{$link}">cookie模式</a>]
<br/><font color="green">(密码模式仅适用于登录不需要验证码的论坛，cookie模式适用于所有论坛)</font><hr/>
<p><label>网站域名:（不要加“http://”）</label><br/>
<input type="text" class="form-control" name="u" value=""/></p>
<p><label>用户名:</label><br/>
...
elseif($_GET['my']=='pwsign2'){
$signurl = ($apiserverid!=0) ? $apiserver.'sign/pwsign/do2.php?backurl='.$baseUrl : '../sign/pwsign2.php';
echo <<<HTML
<div class="w h"><h3>添加PHPWind9.X自动签到任务</h3></div>
<div class="box">
<form action="{$signurl}" method="POST">
[<a href="index.php?mod=signer&my=pwsign&sys={$sysid}{$link}">密码模式</a>][cookie模式]
<br/><font color="green">(密码模式仅适用于登录不需要验证码的论坛，cookie模式适用于所有论坛)</font><hr/>
