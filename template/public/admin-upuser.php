...
{
$c=0;
$rs=$DB->query("SELECT * FROM wjob_user order by userid asc");
while($row = $DB->fetch($rs))
{
$gl=$row['user'];
$num=$DB->count("SELECT count(*) from wjob_job WHERE lx='$gl'");
$ad=$DB->query("update `wjob_user` set `num` ='$num' where `user`='$gl'");
if($ad)$c++;
}
