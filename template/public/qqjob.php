...
if($my=='qqsign'||$my=='scqd'||$my=='vipqd'||$my=='lzqd'||$my=='gamevip'||$my=='payqd'||$my=='mcqd')$pl='18000';
if($my=='qqss')$pl='1800';
if($my=='3gqq')$pl='600';
if($my=='quantu')$pl='600';
...
for($i=0;$i<$conf['sysnum'];$i++){
	...
	$sysselect.='<option value="'.$sysnum.'">系统'.$sysname[$sysid].'-'.$show[$i].'一次 ('.$all_sys.'/'.$conf['max'].')'.$addstr.'</option>';
}
$sysselect.='</select><br/>';
...
echo '<a href="index.php?mod=qqjob&my=vipqd&qq='.$qq.$link.'"><img src="images/icon/chen.ico" class="logo">添加QQ会员自动签到任务</a><hr/>';
...
<div class="row well">
<font color="blue">提示：<br/>QQ会员签到领取成长值，包括面板签到、网页版签到、积分签到与手机版签到，还有每月领取成长值，每周薪水。</font>
</div>
HTML;
}
