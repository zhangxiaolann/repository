...
$row1=$DB->get_row("SELECT *FROM wjob_job where jobid='{$jobid}' limit 1");
if($isadmin==1)$gl=$row1['lx'];
$sysid=$row1['sysid'];
}

if ($_GET['my'] == 'kq') {//任务开启暂停
	$sds = $DB->query("update `wjob_job` set `zt` ='0' where `jobid`='{$jobid}'");
	if ($sds) {
		showmsg('开启成功!',1,'rw');
	} else {
...
if ($_GET['my'] == 'zt') {
	$sds = $DB->query("update `wjob_job` set `zt` ='1' where `jobid`='{$jobid}'");
	if ($sds) {
		showmsg('暂停成功!',1,'rw');
	} else {
...
if($siteurl!=$conf['siteurl'])
	$DB->query("update `wjob_config` set `siteurl`='$siteurl' where `id`='1'");
...
$rowm1=$DB->get_row("SELECT * FROM wjob_job WHERE proxy='{$qq}' and mc='{$mc}' limit 1");
if($jobid=='' && $rowm1['url']==''){
if($sysid==-1){showmsg('添加任务失败！<br>错误原因:<br>该系统任务数量已饱和，请重新选择一个系统！',3);
exit();
}
$servernum2=$DB->count("SELECT count(*) FROM wjob_job WHERE sysid='{$sysid}'");
if($servernum2<$conf['max']){}else{showmsg('添加任务失败！<br>错误原因:<br>该系统任务数量已饱和，请重新选择一个系统！',3);
exit();
}
$sql18="insert into `wjob_job` (`sysid`,`mc`,`type`,`url`,`post`,`lx`,`timea`,`timeb`,`usep`,`proxy`,`start`,`stop`,`pl`) values ('".$sysid."','".$mc."','".$type."','".$url."','0','".$gl."','".$date."','".$date."','0','".$proxy."','".$start."','".$stop."','".$pl."')";
$sds=$DB->query($sql18);
if($sds){
...
$sql18="update `wjob_job` set `sysid` ='$sysid',`mc` ='$mc',`url` ='$url',`start`='$start',`stop`='$stop',`pl`='$pl' where `jobid`='$jobid'";
$sds=$DB->query($sql18);
if($sds){
...
$rowm1=$DB->get_row("SELECT * FROM wjob_job WHERE url='{$url}' limit 1");
...
$sql18="insert into `wjob_job` (`sysid`,`mc`,`type`,`url`,`post`,`postfields`,`cookie`,`lx`,`timea`,`timeb`,`usep`,`proxy`,`referer`,`useragent`,`start`,`stop`,`pl`) values ('".$sysid."','".$mc."','".$type."','".$url."','".$post."','".$postfields."','".$cookie."','".$gl."','".$date."','".$date."','".$usep."','".$proxy."','".$referer."','".$useragent."','".$start."','".$stop."','".$pl."')";
...
if(!$mc)$mc='批量添加的任务';
$rowm1=$DB->get_row("SELECT * FROM wjob_job WHERE url='{$val}' limit 1");
if($rowm1['url']==''){
$sql18="insert into `wjob_job` (`sysid`,`mc`,`url`,`lx`,`timea`,`timeb`,`start`,`stop`,`pl`) values ('".$sysid."','".$mc."','".daddslashes($val)."','".$gl."','".$date."','".$date."','".$start."','".$stop."','".$pl."')";
...
$sql18="DELETE FROM wjob_job WHERE jobid='$jobid'";
$sds=$DB->query($sql18);
if($sds){
showmsg('任务删除成功!',1,'rw');
}else{
...
$url2=$DB->count("SELECT count(*) FROM wjob_job WHERE url='{$url}' ");
