...
if($DB->query("DELETE FROM wjob_job WHERE lx='$gl' and sysid='{$sysid}'")==true){
echo '清空成功，';
}else{
...
if($DB->query("DELETE FROM wjob_job WHERE lx='$gl' and type='2'")==true){
echo '清空成功，';
}else{
echo'清空失败，<br/>'.$DB->error().'</div>';
exit;
...
$rs=$DB->query("SELECT qq FROM wjob_qq WHERE lx='{$gl}'");
while($myrow = $DB->fetch($rs)){
	$qq=$myrow['qq'];
	$DB->query("DELETE FROM wjob_job WHERE proxy='$qq'");
}
if($DB->query("DELETE FROM wjob_qq WHERE lx='{$gl}'")==true){
$DB->query("UPDATE wjob_user SET qqnum= '0' WHERE user = '$gl'");
echo '清空成功，';
}else{
echo'清空失败，<br/>'.$DB->error().'</div>';
exit;
...
if($DB->query("DELETE FROM wjob_job WHERE proxy='$qq'")==true){
echo '清空成功，';
}else{
echo'清空失败，<br/>'.$DB->error().'</div>';
exit;
...
elseif($_GET['my']=='mm'){
echo '<form action="index.php" medhod="get"><input type="hidden" name="my" value="mm2"><input type="hidden" name="mod" value="set">
...
if($row['pass']==$m){
$sql18="update `wjob_user` set `pass` ='$mm' where `user`='$gl'";
$sds=$DB->query($sql18);
if($sds){
...
elseif($_GET['my']=='mail'){
echo '<form action="index.php" medhod="get"><input type="hidden" name="my" value="mail2"><input type="hidden" name="mod" value="set">
<div class="form-group">
...
<font color="green">提交邮箱后，当sid失效时可以通过邮箱通知您。如果不想被邮箱通知请留空并提交。</font><br/><br/>
...
$sql18="update `wjob_user` set `email` ='$email' where `user`='$gl'";
$sds=$DB->query($sql18);
if($sds){
showmsg('修改成功！',1);
}else{
