...
if ($isadmin==1) { 
if($id!=''){$DB->query("DELETE FROM wjob_chat WHERE id='$id'"); 
...
if($idp!=''){$DB->query("update `wjob_chat` set `nr` ='此楼层已被和谐。。' where `id`='$idp'"); 
...
if($nr==''){}else{ 
	...
	$row22 = $DB->get_row("SELECT * FROM wjob_chat WHERE user='$user' and nr='$nr' LIMIT 1");
	if ($row22!='') {
		echo'<div class="row">';
		echo '不要发重复内容哦!';
		echo "</div>";
	...
	$ipcount=$DB->count("SELECT count(*) FROM wjob_chat WHERE `sj`<'$timelimits' and `ip`='$clientip' limit ".$iplimit);
	if($ipcount>=$iplimit) {	
...
$sql="insert into `wjob_chat` (`user`,`nr`,`sj`,`to`,`ip`) values ('".$usera."','".$nr."','".$date."','".$to."','".$clientip."')";
if($DB->query($sql)){
...
$pagesize=15;
$numrows = $DB->count("select count(*) from wjob_chat where 1");
...
$chatcont=$DB->query("select * from wjob_chat order by id desc limit $offset,$pagesize");
$i=0;
