...
$gls=$DB->count("SELECT count(*) from wjob_qq WHERE 1");
$gxsid=$DB->count("SELECT count(*) from wjob_qq WHERE status!='1'");
} else {
$gls=$DB->count("SELECT count(*) from wjob_qq WHERE lx='{$gl}'");//更新任务总数
if($row['qqnum']==$gls){}else{
$DB->query("UPDATE wjob_user SET qqnum= '$gls' WHERE user = '$gl'");}
...
echo '<ul class="nav nav-tabs">
	  <li class="'.checkIfActive("qqlist").'" ><a href="#">ＱＱ管理</a></li>
	  <li class="'.checkIfActive("wall").'" ><a href="index.php?mod=wall">ＱＱ墙</a></li>
</ul>';
...
首先添加一个QQ账号，添加完成后回到本页面，点击你的QQ号码进入任务列表，在任务列表里可以添加秒赞、秒评、自动说说、QQ机器人等QQ挂机服务。<br/>以后要经常来此页面更新失效的SID&Skey。<br/><font color="red">你需要先获取P_skey，否则发说说等功能无法使用！</font><br/>
...
if(isset($_GET['super']) && $isadmin==1) {
	if(isset($_GET['qq']))
		$rs=$DB->query("SELECT * FROM wjob_qq WHERE qq='{$_GET['qq']}' order by id desc limit $pageu,$pagesize");
	else
		$rs=$DB->query("SELECT * FROM wjob_qq WHERE 1 order by id desc limit $pageu,$pagesize");
	$link.='&super=1';
} else
...
$pagesl = $i + ($page - 1) * $pagesize;
  echo '<tr><td style="width:40%;" id="menubar"><a href="index.php?mod=list&m=qq&qq='.$myrow['qq'].$link.'" title="进入任务管理"><img src="http://q2.qlogo.cn/headimg_dl?bs=qq&dst_uin='.$myrow['qq'].'&src_uin='.$myrow['qq'].'&fid='.$myrow['qq'].'&spec=100&url_enc=0&referer=bu_interface&term_type=PC" class="qqlogo" width="80px"><label>'.$myrow['qq'].'</label></a>';
  if(isset($_GET['super']) && $isadmin==1)echo '<br/>所属用户:<a href="index.php?mod=admin-user&kw='.$myrow['lx'].$link.'">'.$myrow['lx'].'</a>';
...
if (empty($myrow['pskey']))
echo '<br/><font color="red">未获取P_skey！</font>';
elseif ($myrow['status2'] == '1')
echo '<br/><font color="green">P_skey正常！</font>';
else
echo '<br/><font color="red">P_skey已失效！</font>';
