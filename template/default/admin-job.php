...
$numrows=$DB->count("select count(*) from wjob_job where".$sql);
...
$rs=$DB->query("select * from wjob_job where{$sql} order by jobid desc limit $offset,$pagesize");
$i=0;
...
if($myrow['type']==3){
  ...
  echo '</b><br/>建立者:<a href="index.php?mod=admin-user&kw='.$myrow['lx'].'">'.$myrow['lx'].'</a><br/>'.$qqjob['info'];
} else {
  echo '</b><br/>建立者:<a href="index.php?mod=admin-user&kw='.$myrow['lx'].'">'.$myrow['lx'].'</a><br><a href="'.$myrow['url'].'" target="_blank">'.$myrow['url'].'</a><br>';
