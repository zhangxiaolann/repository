...
function get_formhash($res){
	if(preg_match('/name="formhash" id="formhash" value=\'(.*?)\'/i', $res, $matches))
        return $matches[1];
	elseif(preg_match('/name="formhash" value="(.*?)"/i', $res, $matches))
		return $matches[1];
	else
		return false;
}
...
function get_loginhash($res){
	if(preg_match('/loginhash=(.*?)&amp;/i', $res, $matches))
        return $matches[1];
	elseif(preg_match('/loginhash=(.*?)"/i', $res, $matches))
        return $matches[1];
	else
		return false;
}
...
<link rel="stylesheet" type="text/css" href="../../assets/css/getcookie.css">
</head>
<body>
';
}
...
if(!preg_match("/^[a-f0-9]+$/",$_GET['cookie']) || strlen($_GET['cookie'])!=16)exit('Forbidden!');
if(!preg_match("/^[a-zA-Z0-9:.\/]+$/",$_GET['u']) || strlen($_GET['u'])>25)exit('Forbidden!');
//存放Cookies的文件
...
if((preg_match('!charset=gbk\"!i', $res) || preg_match('!charset=\"gbk\"!i', $res)) && !preg_match('!charset=utf-8"!i', $res))$gbk=1;else $gbk=0;
if($gbk){echo $res=iconv('gbk', 'UTF-8//IGNORE', $res);}

preg_match('!<div class="jump_c">(.*?)<p><a class="grey"!is',$res,$content);
if(strpos($res, '欢迎您回来')) {
	//修改cookie的时间
	...
	echo $content[1].'<p><a href="?u='.$_GET['u'].'&cookie='.$_GET['cookie'].'">[ 点此返回上一页 ]</a></p>';
}else echo '获取失败！可能因对方网站的安全防护措施导致无法抓取页面，请刷新此页面重试。';
foot();
exit;
}
...
head();
if(!$formhash){
	echo '<div class="title">获取失败！</div>
	1、对方网站暂无法访问。<br/>
	2、对方网站可能不是DZ论坛。<br/>
	3、可能因对方网站的安全防护措施导致无法抓取页面，请刷新此页面重试。';
	foot();
	exit;
}
