...
function ptuiCB(code,uin,sid,skey,pskey,superkey,nick){
	...
	switch(code){
		case '0':
			...
			ajax.get("index.php?mod=findpwd&act=qrlogin&r="+Math.random(1), 'json', function(arr) {
				if(arr.code==1) {
...
function loadScript(c) {
	...
	c = c || "qq/getsid/qrlogin.php?do=qqlogin&findpwd=1&qrsig="+decodeURIComponent(qrsig)+"&r=" + Math.random(1);
