...
function install_check() {
	if(!ini_get('acl.app_id') && !defined("SAE_ACCESSKEY") && file_exists("job.lock"))return true;
	else {
		if(!file_exists('../config.php'))return false;
		include_once '../config.php';
		if(!$user||!$pwd||!$dbname)return false;
		$cn = mysql_connect($host,$user,$pwd);
		if (!$cn)return false;
		$sl = mysql_select_db($dbname,$cn);
		if (!$sl)return false;
		mysql_query("set names utf8",$cn);
		if(mysql_query("select * from wjob_config where id='1'",$cn)==FALSE)return false;
	}
	return true;
}

echo'<div class="w h">安装彩虹网址监控-阉割版</div><div class="box">';
...
if($do=='5') {
echo <<<HTML
</div><div class="w h">网站信息配置</div><div class="box"><form action="{$_SERVER['PHP_SELF']}?do=6" method="POST">管理员账号(ID=1):<br><input type="text" name="user" value="" maxlength="32"><br>管理员密码:<br><input type="text" name="mm" value="" maxlength="32"><br>网站名称:<br><input type="text" name="sitename" value="免费监控站"><br><input type="submit"
value="保存配置"></form>
HTML;
}
...
$ad=$DB->query("insert into `wjob_user` (`pass`,`user`,`date`,`last`) values ('".$pa."','".$gl."','".$date."','".$date."')");
$ad2=$DB->query("update `wjob_config` set `sitename` ='".$sitename."',`build` ='".$date."',`css2` ='2' where `id`='1'");
if($ad2){
@file_put_contents("job.lock",'安装锁');
//if(preg_match('/(.aliapp.com|.sinaapp.com|.jd-app.com|.mmapp.cn|.mopaas.com|.duapp.com)/',$_SERVER['HTTP_HOST']))
//$DB->query("update `wjob_config` set `apiserver` ='0' where `id`='1'");
echo '<font color="green">安装完成！管理账号和密码是:'.$gl.'/'.$pa.',请在首页进行登录。</font><br/><br/><a href="../">>>网站首页</a><hr/><font color="blue">提示：要想正常运行，需要监控，监控说明请到后台查看。</font><br/>更多设置选项请登录后台管理进行修改。<br/><br/><font color="#FF0033">如果你的空间不支持本地文件读写，请自行在install/ 目录建立 job.lock 文件！</font>';
