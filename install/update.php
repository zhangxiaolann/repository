...
if(!$conf['version'])
{
$sql="ALTER TABLE `wjob_config` 
ADD COLUMN `interval` int(10) NOT NULL DEFAULT 0,
ADD COLUMN `version` int(4) NOT NULL";
if($DB->query($sql)){
$DB->query("update wjob_config set version='2100' where id='1'");
...
$sql2="ALTER TABLE `wjob_job` 
RENAME TO `wjob_job1`";
if($DB->query($sql) && $DB->query($sql2)){
$DB->query("update `wjob_config` set `css` ='1',`sysnum` ='4',`version`='2500' where `id`='1'");
...
{
$sql="ALTER TABLE `wjob_job1`
ADD COLUMN `zt` int(1) NOT NULL DEFAULT 0";
if($DB->query($sql)){
$DB->query("update `wjob_config` set `version`='3000' where `id`='1'");
...
{
$sql="ALTER TABLE  `wjob_user`
ADD  `zcip` VARCHAR( 15 ) DEFAULT NULL ,
ADD  `dlip` VARCHAR( 15 ) DEFAULT NULL";
$sql2="ALTER TABLE  `wjob_chat`
ADD  `ip` VARCHAR( 15 ) DEFAULT NULL";
if($DB->query($sql) && $DB->query($sql2)){
$DB->query("update `wjob_config` set `version`='4200' where `id`='1'");
...
{
$sql="ALTER TABLE  `wjob_config` ADD  `banned` TEXT NULL";
if($DB->query($sql)){
$DB->query("update `wjob_config` set `version`='4300' where `id`='1'");
...
{
$sql="ALTER TABLE  `wjob_config` ADD  `multi` INT(1) NOT NULL DEFAULT '0',
ADD  `loop` VARCHAR(150) NOT NULL DEFAULT '0-0-0-0-0-0-0-0'";
if($DB->query($sql)){
$DB->query("update `wjob_config` set `version`='4600' where `id`='1'");
...
if($do=='1') {
$DB->query("update `wjob_config` `switch` ='0' where `id`='1';");
echo '<div class="box">';
echo'<pre>';
include("update.md");
echo'</pre>';
...
$c=0;
for($i=2;$i<=8;$i++){
$rs=$DB->query("SELECT * FROM wjob_job{$i} order by jobid asc");
while($row = $DB->fetch($rs))
{
if(!$row['url'])continue;
$sql="insert into `wjob_job1` (`sysid`,`url`,`post`,`postfields`,`lx`,`timea`,`timeb`,`usep`,`proxy`,`referer`,`useragent`,`start`,`stop`) values ('".$i."','".$row['url']."','".$row['post']."','".$row['postfields']."','".$row['lx']."','".$row['timea']."','".$row['timeb']."','".$row['usep']."','".$row['proxy']."','".$row['referer']."','".$row['useragent']."','".$row['start']."','".$row['stop']."')";
$DB->query($sql);
$c++;
}
}
$DB->query("ALTER TABLE `wjob_job1` RENAME TO `wjob_job`");
$DB->query("DROP TABLE IF EXISTS `wjob_job2`");
$DB->query("DROP TABLE IF EXISTS `wjob_job3`");
$DB->query("DROP TABLE IF EXISTS `wjob_job4`");
$DB->query("DROP TABLE IF EXISTS `wjob_job5`");
$DB->query("DROP TABLE IF EXISTS `wjob_job6`");
$DB->query("DROP TABLE IF EXISTS `wjob_job7`");
$DB->query("DROP TABLE IF EXISTS `wjob_job8`");
...
$DB->query("update `wjob_config` set `version` ='4000',`switch` ='1',`build` ='$date' where `id`='1'");
