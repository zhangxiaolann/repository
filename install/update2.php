...
ADD `qqblock` text NULL,
ADD `txprotect` int(1) NOT NULL DEFAULT 0,
ADD `txprotect_domain` text NULL,
ADD `getss` int(1) NOT NULL DEFAULT 0";
$sql2="ALTER TABLE  `wjob_qq` ADD  `ptsig` VARCHAR(150) NULL,
ADD  `pskey` VARCHAR(150) NULL,
ADD  `pskey2` VARCHAR(150) NULL";
echo'<div class="box">';
if($DB->query($sql) && $DB->query($sql2)){
$DB->query("update `wjob_config` set `version`='5120' where `id`='1'");
echo 'v5.12数据库更新成功！<br><br><a href="../index.php">>>返回网站首页</a>';
}else{
exit('你已更新过！<br/>'.$DB->error().'<br><a href="../index.php">>>返回网站首页</a>');
}
}
elseif($conf['version']=='5040')
{
$sql="ALTER TABLE `wjob_config`
...
echo'<div class="box">';
if($DB->query($sql)){
$DB->query("update `wjob_config` set `version`='5090' where `id`='1'");
...
{
$sql="ALTER TABLE  `wjob_job` ADD  `cookie` text NULL";
$sql2="ALTER TABLE  `wjob_config` ADD  `sitetitle` text NULL,
...
echo'<div class="box">';
if($DB->query($sql) && $DB->query($sql2)){
$DB->query("update `wjob_config` set `version`='5030',`sitetitle`='-分布式秒赞挂机系统',`mail_stmp`='smtp.163.com',`mail_port`='25',`siteurl`='$siteurl' where `id`='1'");
...
{
$sql="ALTER TABLE `wjob_qq` MODIFY COLUMN  `qq` varchar(20) NOT NULL";
$sql2="ALTER TABLE `wjob_config`
ADD  `cronkey` VARCHAR(150) DEFAULT NULL,
ADD  `qqapiid` INT(4) NOT NULL DEFAULT 0";
echo'<div class="box">';
if($DB->query($sql) && $DB->query($sql2)){
$DB->query("update `wjob_config` set `version`='5010' where `id`='1'");
...
$DB->query("update `wjob_config` set `version` ='5040',`css2` ='2',`sitetitle`='-分布式秒赞挂机系统',`mail_stmp`='smtp.163.com',`mail_port`='25',`siteurl`='$siteurl' where `id`='1';");
